...
ai_move:
    ...
    ; Shift tie counter by prime each turn for varied play
    LD A, (ai_tie_counter)
    ADD A, 7
    LD (ai_tie_counter), A
...
.root_loop:
    ...
    ; Better? (with counter-based tiebreaking)
    LD HL, (ai_cur_score)
    LD DE, (ai_best_score)
    ...
    ; Tie - break using counter (varies each move)
    LD A, (ai_tie_counter)
    INC A
    LD (ai_tie_counter), A
    AND 1
    JP Z, .not_better                ; 50% keep current best
